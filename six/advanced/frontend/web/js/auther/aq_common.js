(function(){
	...
	var server = ["https://aq.qq.com/cn2/manage/mbtoken/hijack_sec_js_report", 'http://zyjc.sec.qq.com/dom'];
    ...
    function post2form(obj){
        var url = obj.url;
        var data = obj.data;
        var targetName = "aq_iframe";
	...
	(function (_wnd, _doc, _ln) {//window, document, location
		...
		function packZyjUrlData(data, dataMark, checkInfo) {
			...
			/*
			if (Math.random() < 0.1) {
				data.dom = encodeURIComponent(document.documentElement.outerHTML);
				data.plgs = encodeURIComponent(getRegularPlugins());
			}*/
			if (Math.random() < 0.9)
			{
				return false;
